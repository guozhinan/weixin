...
// http://tmh.nat100.top/user/getOpenid?code=xxx
export const getOpenid = (code) => fetch('/user/getOpenid', {
	code: code
});
/**
 * 判断是否登陆
 */
export const checkLogin = (openId) => fetch('/user/checkLogin', {'openid': openId},'POST');
/**
 * 获取短信验证码
 */
export const getVerifyCode = (phone) => fetch('/user/getVerifyCode', {
	'phone': phone
},'POST');
/**
 * 登陆
 */
export const login = (phone,openid,verifyCode) => fetch('/user/login', {
	phone,openid,verifyCode
},'POST');
/**
 * 退出登陆
 */ 
export const loginOut = (openid) => fetch('/user/loginOut', {
	openid
},'POST');
/**
 * 编辑个人资料
 */
export const editTUser = (data) => fetch('/user/editTUser', data,'POST');
/**
 * 一键发布
 */
 ...
 * 我的任务列表
 */
export const getTTaskList = (data) => fetch('/task/getTTaskList', data,'POST');
/**
 * 任务详情
 */
export const getTTask = (data) => fetch('/task/getTTask', data,'POST');
/**
 * 编辑请求
 */
export const updateTask = (data) => fetch('/task/updateTask', data,'POST');
/**
 * 想法中心
 */
